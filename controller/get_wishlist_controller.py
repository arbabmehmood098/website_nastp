...
def get_wishlist(wishlist_name):
    ...
    try:
        ...
        return jsonify({"error": f"Failed to retrieve wishlist: {str(e)}"}), 400

    # Retrieve the wishlist data
    try:
        wishlist_data = db.child("siberkoza").child("users").child(user_id).child("wishlist").child(matching_wishlist_id).get().val()
        return jsonify({
            "message": "Wishlist retrieved successfully",
            "wishlist_id": matching_wishlist_id,
